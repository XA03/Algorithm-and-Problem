...
string infix_to_postfix(string infix){
    ...
    for(int i=0;i<infix.size();i++){
        if(isalpha(infix[i]))ret+=infix[i];
            ...
            if(infix[i]=='(')s.push(infix[i]);                      //括號為特殊優先級，直接放入operator stack
            else if(infix[i]==')'){                                 //遇到括號代表還有另一個可配對的括號，把括號中的內容先印出
                while(s.top()!='('){
                    ret+=s.top();
                    s.pop();
                }
    ...
    while(!s.empty()){                                              //一定會有operator沒辦法出來的情況，因為最後一次碰到的operator沒有遇到下一次要比較優先級的時刻
        ret+=s.top();
        s.pop();
    }
